...
public class LambdaStreamExcImp implements LambdaStreamExc {
  ...
  public Stream<String> toUpperCase(String... strings) {
    ...
    // Note: The double colon --> existing class::method in class
    // Notes: map = transform one object to another by applying a function
  }

  /**
   * Filter strings that contains the pattern e.g. filter(stringStream, "a") will return another
   * stream with no elements that contains a
   *
   * @param stringStream
   * @param pattern
   * @return
   ...
   *
   * <p>You will learn: - functional interface http://bit.ly/2pTXRwM & http://bit.ly/33onFig -
   * lambda syntax
   *
   * <p>e.g. LambdaStreamExc lse = new LambdaStreamImp(); Consumer<String> printer =
   * lse.getLambdaPrinter("start>", "<end"); printer.accept("Message body");
   *
   * <p>sout: start>Message body<end
   *
   ...
   *
   * <p>e.g. String[] messages = {"a","b", "c"}; lse.printMessages(messages,
   * lse.getLambdaPrinter("msg:", "!") );
   *
   * <p>sout: msg:a! msg:b! msg:c!
   *
   * @param messages
   * @param printer
   */
   ...
   * methods
   *
   * <p>e.g. lse.printOdd(lse.createIntStream(0, 5), lse.getLambdaPrinter("odd number:", "!"));
   *
   * <p>sout: odd number:1! odd number:3! odd number:5!
   *
   * @param intStream
   * @param printer
   */
  ...
  public Stream<Integer> flatNestedInt(Stream<List<Integer>> ints) {
    return ints.flatMap(value -> value.stream().map(x -> x * x));
  }
}
