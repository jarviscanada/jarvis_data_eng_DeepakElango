...
**Application Developer, CIBC Anti-Money Laundering (2021 [2 months])**: Worked as an Application Developer in a small Agile/Scrum environment where I refactored Java into Scala code and built an Extract-Transform-Load pipeline for the Alert-Ingestion project, where we take high priority alerts from various file sources, transform, and load them into a Database that various parties within the AML department can extract and analyze. Built various test cases for my ETL pipeline using ScalaTest to identify any bugs and test whether the components are doing what they are supposed to do.
